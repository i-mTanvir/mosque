...
export function FeatureSection() {
  ...
  return <section className="feature-section" id="services"><img className="feature-section__image" src="/media/second-section.webp" alt="" aria-hidden="true" loading="lazy" decoding="async" /><div className="feature-section__content"><div className="feature-quote" key={active}><h2>{quote[0]}</h2><p>{quote[1]}<span>{quote[2]}</span></p></div><a className="feature-section__button" href="#pillars"><span className="feature-section__button-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="12" fill="currentColor" /><path d="M10 8.5L16 12L10 15.5V8.5Z" fill="#D9A538" /></svg></span>পবিত্র কুরআন শুনুন</a><div className="feature-section__dots" aria-hidden="true">{quotes.map((_, index) => <span className={index === active ? 'is-active' : ''} key={index} />)}</div></div></section>
}
